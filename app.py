...
log = logging.getLogger("geopal")

# Optional: simple shared-secret via query string (?token=XYZ). Set in Render env.
WEBHOOK_TOKEN = os.environ.get("WEBHOOK_TOKEN")

def _maybe_json(s):
    """Try to decode string as JSON if it looks like JSON; otherwise return as-is."""
...
def data_exchange():
    ...
    # Useful debug lines
    log.info("Content-Type seen: %s", headers.get("Content-Type"))
    ...
    log.info("Form keys: %s", list(fields.keys()))

    # 3) Raw body (safe because cache=True by default)
    raw_body = request.get_data(cache=True, as_text=False)
    raw_len = 0 if raw_body is None else len(raw_body)
    log.info("Raw body length: %s", raw_len)

    # Optional: short preview for logs
    try:
    ...
    # Log to Render (use logging + flush)
    log.info("=== GeoPal Data Exchange Event ===")
    log.info(json.dumps(snapshot, indent=2))
    log.info("Saved meta: %s raw: %s", meta_path, raw_path if raw_body else "(none)")
    sys.stdout.flush()
